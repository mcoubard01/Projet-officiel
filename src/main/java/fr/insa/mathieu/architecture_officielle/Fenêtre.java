 ...
 * @author stard
 */
public class Fenêtre extends Ouverture {
    private final double longueur=1.20;
    ...
    double max_y = Math.max(mur.getDebut().getY(),mur.getFin().getY());
        if (this.getEtage().getHauteur_etage()==mur.getÉtage().getHauteur_etage()){
            if ( (this.getOri_x()>min_x && this.getOri_x()<max_x && this.getOri_y()==mur.getDebut().getY())||(this.getOri_y()>min_y && this.getOri_y()<max_y && this.getOri_x()==mur.getDebut().getX())){
                        ...
                        switch(this.getOrientation()){
                            case 'N' -> {
                                if (this.getOri_y()+this.longueur<max_y){
                                    System.out.println("la fenêtre est comprise dans le mur");
                                    result=true;
                                }
                                else {
                                    System.out.println("la fenêtre dépasse le mur (NORD)");
                                    result=false;
                                }
                            }
                            case 'S' -> {
                                if (this.getOri_y()-this.longueur>min_y){
                                    ...
                                    System.out.println("la fenêtre dépasse le mur (SUD)");
                                    result=false;
                                }
                            }
                        }
                        ...
                        switch(this.getOrientation()){
                            case 'E' -> {
                                if (this.getOri_x()+this.longueur<max_x){
                                    ...
                                    System.out.println("la fenêtre dépasse le mur");
                                    result=false;
                                }
                            }
                            case 'O' -> {
                ...
                System.out.println("L'origine de ta fenêtre ne se trouve pas sur un mur");
                result=false;
            }
        }
        else {
